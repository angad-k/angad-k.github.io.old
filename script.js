	...
	constructor() {
		...
		this.update_color = function () {
			...
			if (this.pointer < this.max_pointer) {
				var r =
					this.colors[this.current_index][0] *
						(1 - this.pointer / this.max_pointer) +
					(this.colors[next_index][0] * this.pointer) /
						this.max_pointer;
				var g =
					this.colors[this.current_index][1] *
						(1 - this.pointer / this.max_pointer) +
					(this.colors[next_index][1] * this.pointer) /
						this.max_pointer;
				var b =
					this.colors[this.current_index][2] *
						(1 - this.pointer / this.max_pointer) +
					(this.colors[next_index][2] * this.pointer) /
						this.max_pointer;
				this.current_color =
					"rgba(" + r + ", " + g + ", " + b + ", 0.9)";
...
function setup_graph_scene() {
	...
	for (var x = 25; x < size_w - 25; x += x_increment) {
		for (
			var y = height_top_bound;
			y < height_bottom_bound - 25.0;
			y += y_increment
		) {
			if (Math.random() < prob) {
